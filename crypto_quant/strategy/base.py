...
from crypto_quant.data.feed import BarData, DataFeed #从数据模块导入单根k线模块和“数据线源”模块
from crypto_quant.enums import OrderSide, OrderStatus, OrderType, PositionSide, TimeInForce, TradingMode #从枚举模块导入一系列的枚举
...
class Account: #账户信息类，一个交易所账户一般包含以下信息
    ...
    maintenance_margin: Decimal = Decimal("0") #维持保证金，合约回测中用于判断是否触发强平
    margin_ratio: Decimal = Decimal("0") #保证金率，合约回测中用于衡量账户风险
...
class Position: #持仓信息类
    ...
    margin: Decimal = Decimal("0") #当前仓位占用保证金，合约回测中使用
    liquidation_price: Decimal | None = None #预估强平价格，合约回测中使用
