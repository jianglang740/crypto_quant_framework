...
它的目标不是直接提供“可盈利策略”，而是提供一套清晰、可扩展、便于学习和二次开发的量化交易工程骨架。你可以用它完成从数据准备、策略开发、单策略回测、多策略组合回测、绩效分析、交互式图表报告、数据库存储，到模拟实盘和真实交易接入的完整开发流程。
...
| 文件 | 用途 |
|---|---|
| `examples/run_backtest.py` | 运行一个最小单策略回测 |
| `examples/run_csv_backtest.py` | 从本地 CSV 加载数据并回测 |
| `examples/run_bokeh_report.py` | 运行回测并生成 Bokeh 报告 |
| `examples/run_live.py` | 运行 dry_run 实盘示例 |
| `examples/run_database_backtest.py` | 演示数据库读写和回测结果保存 |
| `examples/run_market_data_pipeline.py` | 演示 Binance K线拉取、清洗、MySQL入库、读回 |
| `examples/run_live_database_recorder.py` | 演示实盘数据库记录器 |
...
目前示例里使用占位符，真实使用时建议后续扩展为环境变量或 `.env` 读取。
...
```text
1. 增加 pyproject.toml，支持 pip install -e .；
2. 增加 .gitignore；
3. 增加 tests/ 单元测试目录；
4. 增加统一日志模块；
5. 增加 .env 配置读取；
