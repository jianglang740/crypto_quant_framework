...
`PortfolioBacktestEngine` 采用和单策略回测一致的回测假设：

```text
当前 K 线收盘后产生信号；
新订单保持 OPEN，不在当前 K 线立即成交；
下一根 K 线开始时撮合 OPEN 订单；
市价单按下一根 K 线 open 成交。
```

组合策略仍然按传入列表顺序执行。当前脚本中先执行趋势策略，再执行趋势过滤反转策略，因此反转策略可以读取趋势策略更新后的 `trend_state`。
