...
| 字段 | 类型 | 含义 |
|---|---|---|
| `cash` | `Decimal` | 现金余额；合约模式下可以理解为钱包余额，不包含未实现盈亏 |
| `equity` | `Decimal` | 账户总权益；合约模式下约等于 `cash + unrealized_pnl` |
| `available` | `Decimal` | 可用资金；合约模式下约等于 `equity - margin` |
| `margin` | `Decimal` | 已占用保证金，主要用于合约 |
| `maintenance_margin` | `Decimal` | 维持保证金，合约回测中用于判断是否触发强平 |
| `margin_ratio` | `Decimal` | 保证金率，当前简化为 `maintenance_margin / equity` |
...
| `margin` | `Decimal` | 当前仓位占用保证金，合约回测中使用 |
| `liquidation_price` | `Decimal | None` | 预估强平价格，合约回测中使用 |
| `unrealized_pnl` | `Decimal` | 未实现盈亏 |

### 5.3 合约字段说明

`margin` 和 `liquidation_price` 主要由回测引擎维护，策略一般不需要自己手动修改。

例如合约开仓后：

```text
position.margin = 这笔仓位占用的保证金
position.liquidation_price = 根据开仓价、杠杆和维持保证金率估算的强平价格
```

策略可以读取这些字段做风险判断，但真正的保证金占用、释放、强平检查，仍然由 `BacktestEngine` 负责。
